...
#   curl -fsSL https://raw.githubusercontent.com/kfet/slack-acp/main/install.sh | VERSION=v0.5.0 sh
#   curl -fsSL https://raw.githubusercontent.com/kfet/slack-acp/main/install.sh | BIN_DIR=$HOME/bin sh
#
...
#   REPO          github owner/repo (default: kfet/slack-acp)
...
#
# On macOS prefer `brew install kfet/ai/slack-acp` — it upgrades with the rest
# of your system. This script is for Linux hosts (Raspberry Pi included).

set -eu

REPO="${REPO:-kfet/slack-acp}"
BIN_NAME="slack-acp"
...
case "$VERSION" in
	latest) ;;
	''|[!A-Za-z0-9]*|*[!A-Za-z0-9._+-]*) die "bad VERSION '$VERSION': want a release tag, e.g. v0.5.0" ;;
esac
...
printf '\nnext:\n'
printf '  %s\n' 'slack-acp init             # tokens + config.json + env file'
printf '  %s\n' 'slack-acp install-service  # systemd-user unit / launchd agent'
